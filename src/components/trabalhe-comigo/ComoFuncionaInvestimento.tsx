    ...
    <section id="investimento" className="scroll-mt-20 border-t border-white/10 py-20" style={{ backgroundColor: "#101018" }}>
      <div className="mx-auto max-w-7xl px-6">
        ...
        >
          ...
          <div
            aria-hidden="true"
            className="pointer-events-none absolute right-[-62px] top-[30px] w-[240px] rotate-45 bg-amber-400 py-1.5 text-center font-mono text-[10px] font-bold uppercase tracking-wider text-rose-950 shadow-lg"
          >
          ...
          <p className="mt-2.5 font-mono text-[13px] text-rose-100">
            {t({ pt: "ou em até ", en: "or up to " })}
            {PRECO_PARCELA}
          </p>

          <p className="mx-auto mt-5 max-w-md text-[13px] leading-relaxed text-rose-100/80">
            {t({
          ...
          <a
            ...
            className="mt-7 inline-flex items-center gap-2 rounded-full bg-white px-8 py-3.5 text-sm font-semibold text-rose-700 shadow-lg shadow-black/20 transition-all hover:scale-[1.03] hover:bg-rose-50"
          >
        ...
        <div className="mb-3.5 mt-10 text-center font-mono text-[11px] uppercase tracking-wide text-rose-300">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          {PLANS.map((plan) => (
            ...
            >
              {plan.flag && (
                <span className="absolute -top-3 right-4 inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-400 to-rose-400 px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-wide text-rose-950 shadow-[0_0_20px_-2px_rgba(245,158,11,0.9)] ring-1 ring-white/40">
              ...
              <div
                className={`font-mono text-xs uppercase tracking-wide ${
                  plan.featured ? "text-rose-200" : "text-zinc-400"
                }`}
              >
                {t(plan.name)}
              </div>
              <div className="mt-3.5 text-3xl font-bold text-white">
                ...
                <small
                  className={`font-mono text-sm font-normal ${
                    plan.featured ? "text-rose-200/80" : "text-zinc-400"
                  }`}
                >
                  {t(plan.priceSuffix)}
                </small>
              ...
              <div
                className={`mt-1 font-mono text-[11px] ${
                  plan.featured ? "text-amber-200" : "text-rose-300"
                }`}
              >
                {t(plan.note)}
              </div>
                ...
                {plan.items.map((item) => (
                  <li
                    ...
                    className={`relative pl-4 text-sm ${
                      plan.featured ? "text-rose-50" : "text-zinc-400"
                    }`}
                  >
                    <span className={`absolute left-0 ${plan.featured ? "text-amber-300" : "text-amber-400"}`}>
                      •
                    </span>
                    {t(item)}
                  </li>
